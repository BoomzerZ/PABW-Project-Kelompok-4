<template>
  <div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden max-w-sm relative group">
    ...
    <img :src="product.image_url" :alt="product.name" class="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-500 cursor-pointer" @click="$router.push('/product/' + product.id)" />
    <div class="p-4">
      <router-link :to="'/product/' + product.id">
        <h3 class="text-lg font-bold text-white hover:text-red-500 transition-colors cursor-pointer">{{ product.name }}</h3>
      </router-link>
